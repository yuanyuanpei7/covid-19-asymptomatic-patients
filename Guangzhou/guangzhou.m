...
N0=3e6; %total pupulation take into
E0=Ipresent(1,1);
I0=Ipresent(1,1);
R0=Ipresent(1,2);
A0=Ipresent(1,3);
...
title('Guangzhou')
firstInfect=c1*beta*exp(-decays*t)*Pi;
secondInfect=c2*beta*exp(-decays*t)*(1-Pi); 
...
firstresult=c1*beta*exp(-decays*t)*Pi.*p(:,1).*p(:,2)/N;
secondresult=c2*beta*exp(-decays*t)*(1-Pi).*p(:,1).*p(:,2)/N;
title('Guangzhou')
